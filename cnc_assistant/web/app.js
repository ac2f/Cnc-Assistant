      ...
      <div class="arac">
        <div class="grup"><label>Baslangic (kopma) destek yonu</label>
          <select class="alan" id="d_destek" style="width:170px">
            <option value="sag-ust">Sag-ust (varsayilan)</option>
            <option value="ust">Orta-ust</option>
            <option value="sol-ust">Sol-ust</option>
          </select></div>
        ...
        <div class="grup"><label>Kayit adi (bos=önek)</label>
          <input type="text" class="alan kk" id="d_kayit_ad" placeholder="${(v.dosya_kok||'')}_optimized.dxf" style="width:200px"></div>
      ...
      <div class="arac" id="d_rapor_kutu" style="margin-top:10px;flex-direction:column;align-items:stretch;gap:8px;border-top:1px solid var(--kenar);padding-top:10px">
        ...
        <div id="d_rapor_govde" style="display:none;flex-direction:column;gap:8px">
          ...
          <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
            ...
            <input type="text" class="alan kk" id="d_rapor_ad" placeholder="rapor adi (bos=önek)" style="width:200px">
            <button class="dugme" id="d_rapor_indir">Hata raporu indir (.json)</button>
          </div>
          <div id="d_rapor_durum" class="kk2"></div>
        </div>
      </div>
  ...
  R.secim.forEach((d, handle) => {
    ...
    h += `<div class="rrow ${akt?'aktif':''}" data-h="${handle}">
      ...
      <div class="rr2">
        ...
        <button class="dugme hayalet kucuk" data-sil="${handle}">✕</button>
      </div></div>`;
  });
  kap.innerHTML = h;
  kap.querySelectorAll("[data-akt]").forEach(b => b.onclick = () => {
    R.aktifHandle = b.dataset.akt; R.isaret = true; dxfCiz(doc); dxfRaporListe(doc); });
  kap.querySelectorAll("[data-sil]").forEach(b => b.onclick = () => {
    R.secim.delete(b.dataset.sil); if (R.aktifHandle===b.dataset.sil) R.aktifHandle=null;
    dxfCiz(doc); dxfRaporListe(doc); });
  kap.querySelectorAll("[data-not]").forEach(inp => inp.onchange = () => {
...
function gcodeIcerik(doc) {
  ...
  el.innerHTML = `
    <div class="kart">
      <div class="gc-arac">
        ...
        <label class="anahtar"><input type="checkbox" id="g_karsi" ${doc.gc.karsilastir?"checked":""}>
          <span class="kutu"></span> Orijinal katman</label>
        <span class="ayrac"></span>
          ...
          <div id="g_rapor_kutu" style="margin-top:8px;border-top:1px solid var(--kenar);padding-top:8px">
            ...
            <div id="g_rapor_govde" style="display:none;flex-direction:column;gap:8px;margin-top:8px">
              ...
              <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
                ...
                <input type="text" class="alan kk" id="g_rapor_ad" placeholder="rapor adi (bos=önek)" style="width:180px">
                <button class="dugme" id="g_rapor_indir">Hata raporu indir (.json)</button>
              </div>
              <div id="g_rapor_durum" class="kk2"></div>
            </div>
          </div>
  ...
  setTimeout(() => {
    ...
    $("g_karsi").onchange = e => { doc.gc.karsilastir = e.target.checked;
      AYAR.yaz("gcKarsi", e.target.checked); gcSvg(doc); };
...
function gcGrupCiz(doc) {
  ...
  if (doc.gc.rapor && doc.gc.rapor.aktif) gcRaporListe(doc);
}
...
// Cok-secim: tikla=tekli, Ctrl/Cmd=ekle/cikar, Shift=aralik (capadan).
function gcSecimTik(doc, poz, e) {
  const sira = doc.gc.sira, id = sira[poz], sec = doc.gc.secim;
  if (e.shiftKey && doc.gc.capa != null) {
    const a = sira.indexOf(doc.gc.capa);
    const lo = Math.min(a, poz), hi = Math.max(a, poz);
    if (!(e.ctrlKey || e.metaKey)) sec.clear();
    for (let i = lo; i <= hi; i++) sec.add(sira[i]);
  } else if (e.ctrlKey || e.metaKey) {
    if (sec.has(id)) sec.delete(id); else sec.add(id);
    doc.gc.capa = id;
  } else {
...
function gcGrupTasi(doc, hedefPoz) {
  const sira = doc.gc.sira, sec = doc.gc.secim;
  const secliIdler = sira.filter(id => sec.has(id));   // mevcut sirada
  if (!secliIdler.length) return;
  const hedefId = sira[hedefPoz];
  if (sec.has(hedefId)) return;                        // secimin icine birakma
  const kalan = sira.filter(id => !sec.has(id));
  let ins = kalan.indexOf(hedefId);
  if (ins < 0) ins = kalan.length;
  const yeni = kalan.slice(0, ins).concat(secliIdler, kalan.slice(ins));
...
function gcListe(doc, ihl) {
  ...
  doc.gc.sira.forEach((id, poz) => {
    ...
    const oPoz = doc.gc.orijinal.indexOf(id);          // orijinal sira
    const delta = oPoz - poz;                           // + => oncelendi
    ...
    d.ondrop = e => { e.preventDefault();
      if (sec.size > 1) { gcGrupTasi(doc, poz); return; }
...
function cizVarliklar(svgId, varliklar, riskliHandlelar, basGoster, rapor){
